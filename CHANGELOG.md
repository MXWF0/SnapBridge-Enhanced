...
- 生成保持 v1.2 升级兼容的签名包 `workspace/build/SnapBridge-Enhanced-v1.3.apk`（SHA-256：`6E149A7AFC65708ECC3BC58080FD191824D846E303A723EE670593B10E9519FF`），v1/v2/v3 校验通过。沿用 v1.2 的本地测试证书，已在设备上完成 v1.2→v1.3 覆盖安装；该证书不是生产签名，私钥未上传 GitHub，签名 APK 已上传至 [v1.3 Release](https://github.com/MXWF0/SnapBridge-Enhanced/releases/tag/v1.3)。
...
- 已用 Apktool 2.11.1/AAPT2 构建未签名 APK，并由本地 release 密钥生成签名包；已推送 `main` 并创建正式 [v1.3 GitHub Release](https://github.com/MXWF0/SnapBridge-Enhanced/releases/tag/v1.3)，未修改 Nikon BLE 认证、PTP、JNI 或 Native `.so`。真实相机回归仍待用户现场确认。
...
- 修复蓝牙 AutoLink 的尺寸解析：`IMAGE_ORIGINAL` 不再因未命中内部映射而被解析成 `IMAGE_2MP`，而是沿现有 PTP 控制器真正请求原图；自动传输失败时不再回退请求 2MP，沿既有失败通知明确提示。8MP 按相机能力门控，Remote/手动 Original 仍调用现有 Wi-Fi Direct/PTP 原图链路，并记录请求/保存/解析/实际尺寸。
- 统一 BLE、认证、BTC、Wi-Fi/PTP 和 Remote 的状态清理；断开、深度休眠和重连会话不会继续显示旧 PTP、电量、剩余张数或传输统计；电量查询增加会话校验、5 秒超时和有限重试。
- 分离相机/手机低电量、省电模式和手动暂停状态；手动暂停不会被自动恢复，省电原因未全部解除时不会恢复，断线不发送 resume，当前文件沿用既有边界完成后再暂停；相机低电量继续显示通知。
- 主页和通知仅显示真实连接方式、暂停原因和传输数量；黄色提示只在必要的认证、传输或重连过渡状态短暂显示。修复常驻通知只停留“等待相机”：在服务进程的 `S7.b()` 连接状态变更入口重新提交前台通知，正文从 `CameraService` 的 `ActiveCameraConnectionStatus` 读取实时 BLE/PTP 链路；保留 `ONLY_ALERT_ONCE`，避免每次状态刷新反复提醒。
- 更新器从 PackageManager 读取版本，稳定版与 prerelease 分开比较，并在 Activity 销毁后阻止更新对话框继续弹出。
- LiveView 单击 AF 改为单击确认后触发，与双击变焦、双指缩放分离；未新增 Remote 功能，未修改 Nikon BLE/PTP/JNI/Native `.so` 层。

### 最终构建与验证

- 最终未签名构建输入 `workspace/build/base-unsigned.apk`：`68938393` 字节，SHA-256 `3A32E91BE2077FA8BE8064311997D176F94EE7FE4832B2AD9615A777A6F278BD`。
- 最终签名 APK `workspace/build/SnapBridge-Enhanced-v1.2.apk`：`69157640` 字节，SHA-256 `BC5691E317B4090B5DADE511AD29FEBDF53E289DAC53BC38B8C490FFBEE77E4B`；使用本地临时测试证书，不是生产发布证书。
- `aapt2 dump badging`、编译校验、v1/v2/v3 签名、覆盖安装和 Android 设备冷启动通过；与 v1.1 基线的 16 个 Native `.so` 条目长度和 CRC 一致。覆盖安装后的 V2324A 前台服务无 `NoSuchMethodError`、`VerifyError`、`NoClassDefFoundError` 或 `FATAL EXCEPTION`；无相机时通知显示“等待相机”属于预期空闲态，保持前台服务标记，连接状态切换需现场相机确认。
- 当前设备没有真实 Nikon 相机，2MP、8MP、Original→Wi-Fi、连续传图、锁屏后台、断线重连、低电量暂停恢复、手动暂停和 Remote 进出仍待现场真机确认，未将静态/冷启动验证记为相机功能通过。
